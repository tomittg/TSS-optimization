...
void ICDiffusion(int node, priority_queue<pair<int,int>, comparePairs>& candidateNodes, vector<int>& nodeUpdates, vector<int>& currentDegree);
...
set<int> greed(){
    priority_queue<pair<int,int>, comparePairs> candidateNodes;
    ...
    while(sumActivatedNodes < graph.size()){
        int node = candidateNodes.top();
        condidateNodes.pop();
